...
{
    /// <summary>
    /// Exception that gets thrown when an <see cref="MethodInfo"/> on an <see cref="EventHandler"/> is not asynchronous.
    /// </summary>
    public class EventHandlerMethodMustBeAsynchronous : Exception
    {
        ...
        public EventHandlerMethodMustBeAsynchronous(MethodInfo methodInfo, Type eventType)
            : base($"Handle method for event '{eventType.AssemblyQualifiedName}' on handler '{methodInfo.DeclaringType.AssemblyQualifiedName}' is not asynchronous. Remember to return a Task that can be awaited")
        {
        }
    }
}