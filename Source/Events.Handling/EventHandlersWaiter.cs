                ...
                while (!IsDone())
                {
                    ...
                    if (num-- == 0)
                    {
                        lock (_lockObject)
                        {
                            ...
                            _logger.Warning("Waiting timed out for {WaitingFor} in event handlers {EventHandlers}. Completing waiter with correlation {CorrelationId}", waitingFor, handlers, _correlationId);
                            break;
                        }
                    }
                }
