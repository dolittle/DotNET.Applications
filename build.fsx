...
Target "GenerateAndPublishDocumentation" (fun _ ->
    if String.IsNullOrEmpty(documentationUser) then
        ...
        ProcessHelper.Shell.Exec("git" , args="config --global user.name \"Dolittle Documentation Account\"") |> ignore
        ProcessHelper.Shell.Exec("git" , args="config --global user.email \"docs@dolittle.com\"") |> ignore
