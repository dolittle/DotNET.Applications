...
{
    public class when_getting_module_from_path_with_module_and_two_features : given.a_module_name
    {
        ...
        It should_should_have_first_FeatureDefinition_with_one_sub_feature = () => module_definition.Features.First().SubFeatures.Count().ShouldEqual(1);

        It should_should_have_second_FeatureDefinition_with_correct_name = () => module_definition.Features.First().SubFeatures.First().Name.ShouldEqual((FeatureName)feature2_name);
        It should_should_have_second_FeatureDefinition_with_valid_feature_id = () => module_definition.Features.First().SubFeatures.First().Feature.ShouldNotEqual((Feature)Guid.Empty);
        It should_should_have_second_FeatureDefinition_with_no_sub_features = () => module_definition.Features.First().SubFeatures.First().SubFeatures.Count().ShouldEqual(0);
